...
async function buscarmateria() {
    ...
    const resultado = await db.query('SELECT * FROM materias order by id')

    return resultado.rows;

}

async function deletarmateria(materiaId){    
...
async function salvarmateria(event,materiaNome, materiaDescricao, materiaCursoId){
    event = ''
    const resultado = await db.query('INSERT INTO public.materias(nome, descricao, idcurso)VALUES ($1, $2);', [materiaNome, materiaDescricao, materiaCursoId]);
    return resultado.rows;
}
